...
{
	float2 texCoords = In.TexCoord0.xy;
	texCoords.x = ( texCoords.x + 0.5 ) * 0.5;
	texCoords.y = ( texCoords.y + 0.5 ) * 0.5;
    float depth = Linearize( FetchDepthBufferValue( texCoords.xy ) );
    float depthFucus = Linearize( FetchDepthBufferValue( float2(0.5,0.5) ) );

	depth = MTAUnlerp( sFadeStart, sFadeEnd, depth );
