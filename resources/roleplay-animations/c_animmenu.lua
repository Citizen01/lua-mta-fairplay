...
function ani_start()
	...
	bindKey("space", "down", "Animation stop", "")
end
...
function ani_otworz()
	if (not ani_window) or (not guiGetVisible(ani_window)) then
...
-- Commands
addCommandHandler("Animation open",
	function()
...
addCommandHandler("Animation stop",
	function()
		if (not exports['roleplay-accounts']:isClientPlaying(localPlayer)) then return end
		applyAnimation()
	end, false, false
...
addCommandHandler("animselect",
	function(cmd)
		if (not exports['roleplay-accounts']:isClientPlaying(localPlayer)) then return end
		executeCommandHandler("Animation open")
	end, false, false
)